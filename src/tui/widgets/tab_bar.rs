//! Promoted Warp snapshot (crates/warp_tui/src/tab_bar.rs). Only change from upstream: Warp-harness test module stripped. See research/phase2.md.
...
/// Optional per-tab adornments (dirty markers, counts, drill-down chips).
/// Unused while tabs carry bare titles; kept as part of the snapshot.
#[allow(dead_code)]
impl TuiTab {
...
// Owner-side Warp API for explicit page anchors (orchestration drill-downs).
// Unused while tab counts fit on screen; kept as part of the snapshot.
#[allow(dead_code)]
...
// Overflow-paging surface; exercised when tabs outgrow the width. Kept as
// part of the snapshot with the other paging APIs.
#[allow(dead_code)]
...
// Overflow-paging surface; see `TuiTabBarResolvedPage`.
#[allow(dead_code)]
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
impl TuiTabBarView {
    ...
    // Component query API (covered by widget tests under `cargo test`).
    #[allow(dead_code)]
    ...
    // Component query API (overflow edge jumps; see paging note above).
    #[allow(dead_code)]
    ...
    // Component query API for drill-down levels (unused with flat sessions).
    #[allow(dead_code)]
